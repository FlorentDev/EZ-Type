...
static bool touchesAppuyees[255];
/* Memorise le dernier caractere clavier qui a ete genere */
static GLubyte sCaractereClavier = ' ';
/* Memorise le dernier caractere clavier qui a ete relache */
static GLubyte sCaractereClavierRelache = ' ';
...
/* Valeur de la callback de gestion du clavier relaché */
...
static int *BVR2ARVB(int largeur, int hauteur, const unsigned char *donnees)
{
														...
														pixels de l'image */
	{
		...
		// If the pixel is black, set the alpha to 0
...
/* Renvoie le dernier caractere clavier traite par l'evenement ClavierRelache */
char caractereClavierRelache(void)
{
	return (char)sCaractereClavierRelache;
}
...
//	sAppuiToucheAlt = (glutGetModifiers ()&GLUT_ACTIVE_ALT) != 0;
	...
	touchesAppuyees[caractere] = true;
...
{
	/* On memorise le caractere et la position graphique (et pas fenetre) de la souris pour pouvoir les fournir par la suite */
	sCaractereClavierRelache = caractere;
#ifdef ACTION_CLAVIER_MEMORISE_POSITION_SOURIS
	sAbscisseSouris = xSouris;
	sOrdonneeSouris = sHauteurFenetre-ySouris;
#endif
...
//	sAppuiToucheAlt = (glutGetModifiers ()&GLUT_ACTIVE_ALT) != 0;
	...
	touchesAppuyees[caractere] = false;
